...
3. §4 T6 is said to be green on both sides. On the probe's own fork arm its premise (variable unset) is false, so it carries a `skipif` on `ISOCENTER_HANG_PROBE_START_METHOD == "fork"` rather than being added to the workflow's `--deselect` list.
...
- `iterations` -- `number`, default `10`. Hard ceiling stated in the file: 22 (22 x 15 + 25 = 355 minutes, under the 360-minute job maximum).
...
- `per_iteration_minutes` -- `number`, default `15`. The deadline after which one iteration is declared a hang. Runner suite time is 5m30-6m; the old 900s stall would have fit inside 15 as well, and the current busy timeout ends a lock stall in 120s.
